...
SYSTEM_PROMPT = (
    ...
    "Reusing ids: search_books returns each book's id. When you list books and "
    "the shopper then refers to them (e.g. 'add those', 'add the 3rd one', "
    "'add all of them'), REUSE the ids you already received — do NOT search "
    "again by title. Re-searching by title is unreliable and may miss books "
    "you just showed. Only search again if you genuinely don't have the id. "
    "When adding several books, call add_to_cart once per book using the ids "
    "from your earlier results.\n\n"
    "Currency: all prices are in Indian Rupees. Always format prices with the "
    "₹ symbol (e.g. ₹499.00) — never use $ or 'dollars'. The price values from "
    "the tools are already in rupees; just prefix them with ₹.\n\n"
    "Cart & orders:\n"
